...
DATA_FOLDER = 'merged_299x299_ultimate_3'
...
def run_experiment(tag, optimizer, regularizer = None, dropout = 0.5):
        ...
        params_str = type(optimizer).__name__ + '_lr' + str(K.eval(optimizer.lr)) + '_d' + str(K.eval(optimizer.decay)) + '_l2' + ('None') + '_d' + str(dropout)
        ...
        model = squeezenet.squeezenet(NB_CLASSES)
        #model.load_weights('saved_models/attempt_xception_1py20170206-194318[XCeption]Adam_lr0.001_d0.0_l20.00001_d0.0_2_20-0.91.hdf5')
        ...
        es = EarlyStopping(patience=100, monitor='acc')
        ...
        mc2 = ModelCheckpoint(filepath='saved_models/' + model_name + '_2_' + '{epoch:02d}-{val_acc:.2f}' + '.hdf5', monitor='acc', save_best_only=True, period=3)
        reduce_lr = ReduceLROnPlateau(monitor='acc', factor=0.3, patience=15, verbose=1, epsilon=0.3)
...
#run_experiment('[XCeption]', RMSprop(lr=0.001, decay=0), l2(0.00005), 0.5)
#run_experiment('[XCeption]', RMSprop(lr=0.001, decay=0), l2(0.005), 0.5)
run_experiment('[SQZ]', Adam(lr=0.0005, decay=0), None, 0.0)
